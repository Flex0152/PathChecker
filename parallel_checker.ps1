...
function Test-PathLengths {
    ...
    try {
        ...
        if ($UseParallel) {
                ...
                foreach ($item in $items) {
                    ...
                    if ($pathLength -gt $maxLen) {
                        $results += [PSCustomObject]@{
                            ...
                            Name = $item.Name
                            Parent = if ($item.PSIsContainer) { $item.Parent.FullName } else { $item.Directory.FullName }
            ...
            # Ordner parallel verarbeiten
            if ($CheckFolders) {
                Write-Host "Verarbeite Ordner parallel..." -ForegroundColor Cyan
                ...
                for ($i = 0; $i -lt $folders.Count; $i += $BatchSize) {
                    ...
                    $batchResults = $batch | ForEach-Object -ThrottleLimit $ThrottleLimit -Parallel $scriptBlock -ArgumentList @(,$_), $MaxLength
                    $longPaths += $batchResults
            ...
            if ($CheckFiles) {
                Write-Host "Verarbeite Dateien parallel..." -ForegroundColor Cyan
            ...
            # Sequentielle Verarbeitung (Original-Code)
            # Ordner prüfen
            if ($CheckFolders) {
...
# PowerShell Version prüfen für parallele Verarbeitung
if ($UseParallel -and $PSVersionTable.PSVersion.Major -lt 7) {
    Write-Host "WARNUNG: Parallele Verarbeitung erfordert PowerShell 7+. Verwende sequentielle Verarbeitung." -ForegroundColor Yellow
    $UseParallel = $false
}
...
# Prüfung starten
$results = Test-PathLengths -RootPath $Path -MaxLength $MaxLength -CheckFiles $IncludeFiles -CheckFolders $IncludeFolders -UseParallel $UseParallel -ThrottleLimit $ThrottleLimit -BatchSize $BatchSize
